...
Answer: 
streak length of 1 means made one hit and then missed the shot. In includes one hit and one miss.
streak length of 0 means miss happens without any hit in that streak. so we can tell 0 hit and 1 miss
in a streak length of 0.
...
max(kobe_streak)
```
the distribution seems skewed right with average streak length 0.76, median 0.
his longest streak was 4 hits which means he made 4 shots contineously.
...
Answer:
```{r flip unfair, eval=TRUE}
table(sim_unfair_coin)
```
13 heads and 87 tails came up.
...
```{r simbasked, evil= TRUE}
sim_streak<-calc_streak(sim_basket)
barplot(table(sim_streak))
```

```{r statistic, evil= TRUE}
mean(sim_streak)
median(sim_streak)
max(sim_streak)
```
...
Answer: The distribution is right skewed, with mean 1.06,median 0. the longest streak length is 6.
...
Answer:
I believe with 2nd or 3rd trial, it is going to show similar result, but not the same result.Becausewe are using the same data base, but different randomed number, And 133 shots is enough to make the variance small.
...
Answer: They have similar distributions,but a slightly different streaks numbers may not able to 
be an evidence for kobe's hot hand model fits his shooting patterns.
