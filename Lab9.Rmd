...
Answer:
It is an observational study that doesn't have any controlled or experimented group in this study.
Because of it is an observational study, we cannot determine he causation relationship in this study,
but instead we can see correlation between the dependent and independent variables.
...
```
It is left skewed according to the plot which means most of the students rated the class with 
high score. I expected exactly what the histogram shows because I believe most professors are trying their
best for their classes and students and manage the class efficiently.
...
```
Answer:
According to the scatterplot, I don't see any relationship between age and beauty score.
...
```
Answer:
It seems their are more observations in the data compare to the plot above.
...
```{r}
library(ggplot2)
g<-ggplot(evals,aes(x=bty_avg,y=score))
g<-g+geom_point(position = position_jitter())
g<-g+xlab("Beauty Average")+ylab("Score")
g
```
Answer: Eventhough use jitter for the scatter plot, still doesn't show relationship between
Beauty Average and Score, because jitter helps to see the overlapped points, but seems doesn't help
...
```
Answer:
The summary shows it is statistically significant predictor.
...
```
Answer:
residual models are usually normally distributed,but the histogram shows it is slightly skewed left.
...
```{r}
library(ggplot2)
model1 <- lm(score ~ bty_avg + gender, data = evals)
qqnorm(model1$residuals)
qqline(model1$residuals)

plot(model1$residuals ~ evals$bty_avg)
abline(h = 0, lty = 2)
g<-ggplot(evals,aes(y=model1$residuals,x=evals$gender))
g<-g+geom_boxplot()
g<-g+geom_point()
g
```
...
```
Answer:
yes, it is stll significant, and slope and intercept increased.
...
Answer:
socre = 3.74734 + 0.17239 x beauty average
males tend to have a higher rating according to the blue line.
...
Answer:
I  guess cls_profs as the variable to have the weakest assoication with the evaluation score.
Let's run the model...
...
12. Check your suspicions from the previous exercise. Include the model output
    in your response.
Answer:
the cls_profs has the least association to “scores” since it has the highest p-value in the model.
...
Answer:
The p-value of about 0.11 of etnicity means that it has a weak relationship to scores.
...
```{r}
model_drop <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval + 
...
Answer:
after cls_profs removed from the model,a slight change in the coefficients and significance.
...
```{r}
model_back <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval + 
...
```
The residuals of model_back is not normal.

```{r}
plot(abs(model_back$residuals) ~ model_back$fitted.values)

```
Most of the residual values are close to the fitted values.

```{r}
plot(model_back$residuals ~ c(1:nrow(evals)))

```
The residuals shows that they were randomly gathered.
...
Answer:I don't think so. Since the classes are independent, the evaluation should be 
independent as well.
...
18. Based on your final model, describe the characteristics of a professor and 
    ...
    evaluation score.
Answer:
A male and non-minority professor who has graduated from an American school and teaches a one credit course, has a high beauty average score from the students and in the young age.
