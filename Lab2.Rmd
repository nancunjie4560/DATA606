...
dim(cdc)
```
Answer: There are 20,000 of cases and 9 variables in the data set.

genhlth is categorical(ordinal)
exerany is categorical
hlthplan is categorical
smoke100 is categorical
height is numerical(continuous)
weight is numerical(continuous)
wtdesire is numerical(continuous)
age is numerical(continuous)
...
```{r summary height and age, eval=TRUE}
summary(cdc$height)
70-64
summary(cdc$age)
57-31
...
Answer: The interquartile range for height is 6, for age is 26.

```{r f distributtion gender exerany, eval=TRUE}
table(cdc$gender)/20000
table(cdc$exerany)/20000
```
Answer: For the relative frequency distribution in gender, male shows 48%, compare to 
female shows 52%. For the relative frequency distribution, people who exercised past month
has 75%, compare to 25% who didn't.
...
Answer: There are 9569 males in the sample and about 23% of the sample shows "excellent" 
health condition.
...
Answer: the box plot indicates the better health condition,the lower BMI, and 
the worse health condition, the higher BMI.
```{r bmi smoke}
boxplot(bmi ~ cdc$smoke100)
```
I chose smoke100 to see how it relate to BMI because we all know smoking is bad for health
and I want to know how smoking relate to BMI. The plot shows little difference for smoking or not smoking, but there are some extreme points in the plot.
...
```
The increase in weight increase the wtdesire, which has a positive relationship.
...
class(wdiff)
```
Answer: it is an integer. the 0 wdiff means the person is at the most satisfied weight. the positive wdiff means the person wants to gain more weight, and the negative wdiff means the 
...
boxplot(wdiff)
```
Answer: The statistic summary of wdiff tells us that most of the people in the sample are satisfied with their current weight which is also observable from the plot - narrow and sharp distribution.

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
```{r }
mdw <-subset(cdc,cdc$gender=='m')$wtdesire
fdw <-subset(cdc,cdc$gender=='f')$wtdesire
mw <-subset(cdc,cdc$gender=='m')$weight
fw <-subset(cdc,cdc$gender=='f')$weight
boxplot(mdw - mw, fdw - fw)
...
summary(fdw-fw)
```
men and both women want to lose weight, but compare to female, male has huge outliers.
...
inthesd <- subset(cdc, weight < (meanweight + sdweight) & weight > (meanweight - sdweight))
nrow(inthesd)[1]/nrow(cdc)[1]
```
Answer: we can conclude that about 71% of weights are within the one standard deviation of the mean.
