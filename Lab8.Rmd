...
Answer:
I would choose scatter plot to see the relationship of numerical variables,and it looks like a linear
```{r}
...
Answer:According to the plot, the variables have linear relationship, with positive of stengh of 0.61 which is considered low strength of relationship.
...
Answer: the smallest sum of squares is 123721.9 which is the same number from the previous plot.
...
```
Answer:
The slope tells us that the more home runs a team has the more runs they have which means more successful for the team.
...
```{r}
at_bats <- 5578
predicts <- -2789.2429 + 0.6305 * at_bats
predicts
```
Answer:
Since 727 is greater than 712, therefore, 727 is an overestimate by approximately 15 to 16 runs.
...
Answer:
The residuals randomly scattered top, on, and bottom of the line. It indicates the residuals should be a normal.
...
Answer:
Yes, Eventhough, it is skewed, but the plot shows near normality.
...
8.  Based on the plot in (1), does the constant variability condition appear to 
    be met?
Answer:
Yes, the constant variablity seems met the condition. 
...
```
Answer:I tried with run and hit, to see the relationship. It has a positive linear relationship with R-squared 0.6292.
Answer: 
...
```
Answer:
The hits and run is a better prediction than at bats and run with the R-squared number of 0.629 compare to the 0.3505.
...
Answer:
Among the seven variables, the at_avg is the best predictor for runs. The linear model for bat_avg vs. runs has an R2 of 66%.
...
# R^2 = 37% 
ggplot(mlb11, aes(x = at_bats, y = runs)) + geom_point() + geom_smooth(method = "lm", se = FALSE)
summary(lm(runs ~ at_bats, data = mlb11))

# R^2 = 64%
ggplot(mlb11, aes(x = hits, y = runs)) + geom_point() + geom_smooth(method = "lm", se = FALSE)
summary(lm(runs ~ hits, data = mlb11))

# R^2 = 63%
ggplot(mlb11, aes(x = homeruns, y = runs)) + geom_point() + geom_smooth(method = "lm", se = FALSE)
summary(lm(runs ~ homeruns, data = mlb11))

# R^2 = 17%
ggplot(mlb11, aes(x = strikeouts, y = runs)) + geom_point() + geom_smooth(method = "lm", se = FALSE)
summary(lm(runs ~ strikeouts, data = mlb11))

# R^2 = 0%
ggplot(mlb11, aes(x = stolen_bases, y = runs)) + geom_point() + geom_smooth(method = "lm", se = FALSE)
summary(lm(runs ~ stolen_bases, data = mlb11))

# R^2 = 36%
ggplot(mlb11, aes(x = wins, y = runs)) + geom_point() + geom_smooth(method = "lm", se = FALSE)
...
```{r}
mod_new <- lm(mlb11$runs ~  mlb11$new_obs)
summary(mod_new)
```
```{r}
plot(mlb11$new_obs, mlb11$runs, xlab = 'On Base + Slugging', ylab= 'Runs', col = 'blue')
abline(mod_new, col = 'pink')
```
Answer:
The combination of slugging and base percentage shows as a the best predictor so far with R squared of 0.93.
...
```{r}
hist(resid(mod_new))
plot(mlb11$new_obs, resid(mod_new))
abline(h = 0, lty = 5)
```
```{r}
qqnorm(resid(mod_new))
qqline(resid(mod_new))
```
Answer:
The graph shows most scattors are on the line, which means very few and little residuals. Therefore, the base percentage with slugging is a good predictor for runs.
