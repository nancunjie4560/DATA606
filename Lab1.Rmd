...
```{r plot-girls-vs-years, eval=TRUE}
...
plot(x= arbuthnot$year, y=arbuthnot$girls, type = "l")
```
With ploting girls a long with year, we can observe that girs decrease from 1640 to 1660, but shows dramatically increasing trend from 1660 to 1710.
...
the future.
```{r plot-proportion-boys-over-girls, eval=TRUE}
...
The counts of variables in "present" is much larger than the "arbuthnt", but the boy to girl ratio is very similar through the years, 1.05 in "present" compare to 1.07 in "arbuthnot" in average.
