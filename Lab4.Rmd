...
```{r hist height male,eval=TRUE}
hist(mdims$hgt, main = "Male Height Distribution", xlab = "Male Height in cm", 
    col = "yellow", probability = TRUE)
```

```{r hist height female, eval=TRUE}
hist(fdims$hgt, main="Female Height Distribution", xlab = "Female Height in cm",
     col = "pink", probability = TRUE)
```
Answer:
According to the histogram, we can observe that male height distribution has symmetric shape.On the other hand, 
female has slightly right skewed distribution.
...
```{r hist-height add argument, eval=TRUE}
hist(fdims$hgt, probability = TRUE,ylim = c(0,0.06),main="Female Height Distribution")
x <- 140:190
y <- dnorm(x = x, mean = fhgtmean, sd = fhgtsd)
lines(x = x, y = y, col = "blue")
```
...
```
Answer:
Yes, it appears a nearly normal distribution, and it symmetric at mean and median.
...
```
Answer:
Most of the points fall on the line with errant points toward line, it looks very similar with the previous graph.so, this is also a normal distribution.
...
Answer: 
yes, they look very similar, so that it means the female heights is normally distributed.s

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
```{r}
hist(fdims$wgt)
mean_fwgtmean <- mean(fdims$wgt)
sd_fwgt   <- sd(fdims$wgt)
...
```
Answer:
the weight distribution is right skewed compare to the height distribution.the weight distribution shows 
different shape than the sample probability distribution, so it is not come from a normal distribution.
...
Answer:
```{r difference height probability, eval = TRUE}
prob160 <- pnorm(q = 161, mean = fhgtmean, sd = fhgtsd)
prob171<- pnorm(q = 171, mean = fhgtmean, sd = fhgtsd)
prob171 - prob160

sum(fdims$hgt >= 160 & fdims$hgt <= 170) / length(fdims$hgt)
```

```{r difference weight probability, eval=TRUE}
1-pnorm(q = 55, mean =mean_fwgtmean, sd= sd_fwgt)
...
Answer:
it because D(age) is discrete variable, and the C(elbow) is not a discrete variable. so C has 
many repeats in measurement compare the D is not.
...
```
Answer:
According to the plot, it shows as right skewed and the histogram plot confirmed it.
