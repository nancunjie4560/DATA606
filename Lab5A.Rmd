...
Answer: The sample1 is also right skewed with fat tail.
```{r}
hist(samp1)
```
...
The samp2 mean shows a little bit above the population mean, but closer than the samp1.
According to the code below, we can observe that the sample with size of 1000 has closer mean value than 
the mean of sample size 100. So, the larger sampling size shows more accurate estimate of the population mean.


```{r}
samp2<-sample(area,50)
mean(samp2)
1551.36-1499.69  #samp2 mean - population mean = 51.67
1499.69-1351.86  #population mean - samp1 mean = 147.83
...
Answer:
There are 5000 elements in sample_means50. The distribution is normal and centered at about 1500. 
I believe the distribution will be more normally distributed and the center will be closer to the population mean.
...
Answer:
There are 100 elements in this object and each of element represents a sample mean.
...
Answer:
The center is getting closer to the population mean when the sample size is larger,
and the spread of distribution is getting smaller as the sample size increases.
...
Answer:
According to the sample mean I got, the best estimation for population mean is 17000to 18000so far.
```{r}
my_samp50 <- sample(price, 50)
hist(my_samp50)
mean(my_samp50) #170712
```
...
Answer:
With 50 sample size among the 5000, the mean of the sample is 180731.6 which is very close to the population mean, 180796.1.
As the histogram shows, the sample is normally distributed and centered at around 180000.
...
Answer:
According to the graph, The spread sample size of 150 has narrower distribution than the sample size of 50.
The histogram shows center slightly beyond the 180000. So, I expect the mean sale price is slightly higher than 180000.
...
Answer: Through the sampling distribution from 2 and 3, the larger sample size shows the smaller spread,
and the larger sample size, with smaller spread predicts the true value more accurately.
